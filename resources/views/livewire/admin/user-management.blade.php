<div>
    ...
    <div class="mb-4">
        <div class="relative max-w-sm">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search users..." class="input-glass pl-9">
        </div>
    </div>
    ...
    <div class="space-y-3">
                        ...
                        <div class="grid grid-cols-2 gap-2 sm:grid-cols-3">
                            @foreach ($allRoles as $role)
                                <label class="flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white/70">
                                    <input type="checkbox" wire:model="editingRoles" value="{{ $role }}" class="rounded border-white/20 bg-white/5 text-gold-400 focus:ring-gold-400/40">
        ...
        @empty
            <p class="py-8 text-center text-sm text-white/40">No users found.</p>
        @endforelse
    </div>
